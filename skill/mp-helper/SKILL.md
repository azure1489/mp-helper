...
name: mp-helper
description: >-
  微信公众号（WeChat Official Account）素材上传与草稿箱发布助手。当用户想把文章/图文
  发布或推送到微信公众号、放进公众号草稿箱、上传公众号图片或封面素材、或把（AI 生成的）
  内容准备成公众号图文时使用。能做三件事：① 上传永久图片素材（返回 media_id 与可在正文
  引用的 URL）；② 把标题+正文HTML+封面创建为草稿箱里的图文草稿（返回草稿 media_id）；
  ③ 服务健康检查。一把 API Key 对应一个公众号。仅到草稿箱为止、不做群发；仅支持图片，
  不涉及视频/语音/菜单/模板消息/小程序。触发词：发公众号、公众号草稿、推送图文到草稿箱、
  上传公众号封面/图片素材、WeChat official account draft / 草稿箱 / material。
---
